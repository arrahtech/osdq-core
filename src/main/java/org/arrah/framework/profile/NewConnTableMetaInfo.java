 ...
 * data for the table selected from new connection
 * 
 */
...
public class NewConnTableMetaInfo {
	...
	private  Rdbms_NewConn _newConn; // this will hold new connection

	public NewConnTableMetaInfo(Rdbms_NewConn newConn) {
		_newConn = newConn;
	}
	...
	private  ReportTableModel IndexQuery(int i, int j,
		...
		String s = _newConn.getHValue("Database_Catalog");
		s = "";
		String s1 = _newConn.getHValue("Database_SchemaPattern");
		s = s.compareTo("") != 0 ? s : null;
		s1 = s1.compareTo("") != 0 ? s1 : null;
		Vector<String> vector = _newConn.getTable();
		if (reporttable == null)
	...
	private  ReportTableModel MetaDataQuery(int i, int j,
			ReportTableModel reporttable) throws SQLException {
		String s = _newConn.getHValue("Database_SchemaPattern");
		String s1 = _newConn.getHValue("Database_Catalog");
		s1 = "";
		s1 = s1.compareTo("") != 0 ? s1 : null;
		s = s.compareTo("") != 0 ? s : null;
		Vector<String> vector = _newConn.getTable();
	...
	private  ReportTableModel DataQuery(int i, int j,ReportTableModel reporttable
		...
		String s1 = _newConn.getHValue("Database_Catalog");
		s1 = "";
		String s2 = _newConn.getHValue("Database_DSN");
		s1 = s1.compareTo("") != 0 ? s1 : null;
		s = s.compareTo("") != 0 ? s : null;
		Vector<String> vector = _newConn.getTable();
		String s3 = _newConn.getDBType();
		if (reporttable == null)
		 reporttable  = new ReportTableModel(new String[] { "Table",
		...
		for (int k = i; k < j; k++) {
			...
			for (Enumeration<String> enumeration = vector1.elements(); enumeration
				...
				if (s3.compareToIgnoreCase("oracle_native") == 0)
					_newConn.openConn();
				try {
					for (resultset = _newConn.runQuery(s4); resultset.next();)
				...
				try {
					for (resultset = _newConn.runQuery(s5); resultset.next();)
				...
				try {
					for (resultset = _newConn.runQuery(s6); resultset.next();)
				...
				try {
					for (resultset = _newConn.runQuery(s7); resultset.next();)
				...
				try {
					for (resultset = _newConn.runQuery(s8); resultset.next();)
				...
				try {
					for (resultset = _newConn.runQuery(s9); resultset.next();)
	...
	public  TableRelationInfo getTableRelationInfo(String cat,
		...
		_newConn.openConn();
		dbmd = _newConn.getMetaData();
		if (_newConn.getDBType().compareToIgnoreCase("ms_access") == 0) {
	...
	synchronized public  ReportTableModel queryTable(String query, String tableName, Vector<String> colName) {
		...
		ReportTableModel _rt = new ReportTableModel(new String[] { "Count","Table" });
			QueryBuilder qb = new QueryBuilder(
					_newConn.getHValue("Database_DSN"), tableName, _newConn.getDBType());
			...
			try {	
				synchronized (Rdbms_NewConn.class) {
				_newConn.openConn();
				ResultSet rs = _newConn.runQuery(toQuery);
